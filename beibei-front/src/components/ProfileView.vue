...
const fetchUserData = async () => {
  try {
    ...
    if (authStore.user) {
      ...
      console.log('ProfileView: 使用auth store中的用户数据:', authStore.user)
    }
    ...
    try {
      ...
      // 合并数据，优先使用API返回的数据
      userInfo.value = { ...userInfo.value, ...infoResponse }
      userProfile.value = { ...userProfile.value, ...profileResponse }
      console.log('ProfileView: 获取到API数据:', { infoResponse, profileResponse })
    } catch (apiError) {
      console.warn('ProfileView: 获取详细用户信息失败，使用基础信息:', apiError.message)
