...
import ServiceEditDialog from '@/components/admin/ServiceEditDialog.vue'
import {
  getAdminServices,
  updateServiceStatus,
  updateServiceHot,
...
// 切换热门状态
const handleToggleHot = async (service) => {
  try {
    const newHot = service.hot ? 0 : 1
    const action = service.hot ? '取消热门' : '设为热门'

    await updateServiceHot(service.id, newHot)
    ElMessage.success(`${action}成功`)
    loadServices()
  } catch (error) {
    console.error('更新热门状态失败:', error)
    ElMessage.error(error.message || '更新热门状态失败')
  }
}
