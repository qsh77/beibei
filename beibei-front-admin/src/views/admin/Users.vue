<template>
  <div class="admin-users">
    ...
    <UserDetailDialog
      ...
      @status-change="handleStatusChangeFromDialog"
    />
...
// 从详情对话框中更改状态
const handleStatusChangeFromDialog = async ({ user, status }) => {
  try {
    const action = status === 1 ? '启用' : '禁用'
    await ElMessageBox.confirm(`确定要${action}用户 ${user.phone} 吗？`, '确认操作', {
      type: 'warning'
    })
    await updateUserStatus(user.id, { status, reason: `管理员手动${action}` })
