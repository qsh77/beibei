...
public interface WorkerMapper {
     ...
     * 插入阿姨记录
     */
    @Insert("INSERT INTO worker (user_id, level, years, bio, score, status) " +
            "VALUES (#{userId}, #{level}, #{years}, #{bio}, #{score}, #{status})")
    ...
    @Select("<script>" +
            "SELECT " +
            "w.id, w.user_id, w.level, w.years, w.bio, w.score, w.status, " +
    ...
    @Select("SELECT " +
            ...
            "COUNT(CASE WHEN level = 4 THEN 1 END) as level4_count, " +
            "COUNT(CASE WHEN level = 5 THEN 1 END) as level5_count, " +
